...
function setupMessageListener(client, userId, slot) {
   ...
   * Escopo local à sessão — cada client tem seu próprio Map.
   */
  ...
  client.onAnyMessage(async (message) => {
    ...
    // e o fluxo de candidatura a vaga que usa qualquer tipo de mensagem.
    ...
    // Texto vazio para tipo chat/text — encaminha direto (será descartado
    // internamente por handleIncomingMessage, mas sem atrasar o loop)
    if (!text) {
      await handleIncomingMessage(message, client, userId, slot);
      return;
    }
    ...
    const timer = setTimeout(async () => {
      ...
      logger.info(
        `[📦 BUFFER] Disparando processamento: ${buffer.texts.length} msg(s) de ${phone} → "${combinedText.slice(0, 120)}${combinedText.length > 120 ? '…' : ''}"`,
      );
...
async function handleIncomingMessage(message, client, userId, slot) {
    try {
      ...
      // ── Indicador "digitando..." ──────────────────────────────────────────
      // Ativado antes da chamada à IA e desativado após o envio (ou em erro).
