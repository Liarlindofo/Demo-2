...
export function useStockSession() {
  ...
  // Persiste sempre que sessions muda
  useEffect(() => {
                    ...
                    : {
                        ...
                        itens: cat.itens.map(item =>
                          item.insumoId !== insumoId
                            ? item
                            : { ...item, quantidadeContada: quantidade },
                        ),
                      },
                ),
              },
  ...
  const calcularAlertasReposicao = (session: StockSession) => {
    return session.sessoes.flatMap(cat =>
      cat.itens.filter(
        item =>
