...
export const PizzaCard = ({
  product, storeSlug, onClick, selectMode, selected, onClone, onRename,
  onFotosChange, onDescricaoChange,
}: PizzaCardProps) => {
  ...
  const commitRename = () => {
    ...
    if (trimmed && trimmed !== product.nome) onRename?.(trimmed);
    setIsRenaming(false);
  };

  const cancelRename = () => { setRenameValue(product.nome); setIsRenaming(false); };

  const fotos = product.fotos ?? (product.fotoUrl ? [product.fotoUrl] : []);
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="min-w-0 flex-1">
            ...
            {!selectMode && (
              <DescricaoEditavel valor={product.descricao} onSalvar={onDescricaoChange} />
            )}
          </div>
          {isBebida ? (
        ...
        <div className="flex items-end justify-between">
          <span className="text-2xl font-bold" style={{ color: cmvColor }}>{formatPercent(product.cmvPercent)}</span>
          <span className="text-xs text-gray-500">{product.numIngredientes} ingr.</span>
        </div>
        ...
        {!selectMode && !isRenaming && (onClone || onRename) && (
          <div className="absolute bottom-3 right-3 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"
            ...
            {onRename && (
              <button title="Renomear produto" onClick={() => setIsRenaming(true)}
            ...
            {onClone && (
              <button title="Clonar produto" onClick={onClone}
