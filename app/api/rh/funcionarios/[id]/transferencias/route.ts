...
export async function GET(
  ...
  try {
    ...
    const transferencias = await prisma.rhTransferenciaLoja.findMany({
      where: { funcionarioId: id, userId: dbUser.id },
      include: {
        lojaOrigem: { select: { nome: true } },
        lojaDestino: { select: { nome: true } },
      },
    ...
    console.error('[GET /api/rh/funcionarios/[id]/transferencias]', err);
